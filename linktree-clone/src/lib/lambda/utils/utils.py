...
def get_user(req: dict):
    '''
    Returns the sub identifier of a user given an API Gateway payload

        Parameters:
            req (dict): The payload of the request
        
        Returns:
            username (str): Unique username from Cognito
    '''

    try:
        return req['requestContext']['authorizer']['claims']['cognito:username']
    except KeyError:
